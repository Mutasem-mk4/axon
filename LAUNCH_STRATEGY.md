# Secfacts - Global Launch & Growth Strategy

## 1. Hacker News & Reddit Strategy

### Show HN: secfacts – High-performance security evidence normalization in Go
**Draft:**
...
To solve this, I built **secfacts** (https://github.com/secfacts/secfacts). It's a normalization engine written in Go that uses a **Sharded Actor Model** to deduplicate and correlate security evidence with near-zero overhead.
...
**Title:** [OSS] secfacts: A sharded-actor engine for security alert correlation (Go)
**Draft:**
Hey everyone, I'm releasing `secfacts`, a tool designed to turn "1,000 noisy alerts" into "3 actionable issues." 
...
**We need your help with Stress Testing!** If you have massive (1GB+) SARIF files, try running them through `secfacts` and let us know how the memory footprint holds up.

Check it out: https://github.com/secfacts/secfacts

---

## 2. Technical Blog Post
**Title:** Why we built secfacts: Moving beyond Alert Fatigue with a Sharded-Actor Correlation Engine.
...
https://github.com/secfacts/secfacts #DevSecOps #Golang #CyberSecurity

**Post 2: Call to Contributors**
We built the engine. Now we need the fuel. ⛽

`secfacts` v0.1.0 is out with a high-speed SARIF parser. We're looking for contributors to help us expand our **Parser Registry**. Want to add support for Snyk, SonarQube, or ZAP? 

Our Interface-based Provider pattern makes it easy to add new adapters. Join us: https://github.com/secfacts/secfacts #OpenSource #InfoSec
...
3. **Evidence Baselines (Drift Detection):** Implement a baseline feature (`--save-baseline`) that allows secfacts to ignore "Known & Accepted" risks and only fail builds on **new** or **regressed** vulnerabilities.
